...
The results in Figs.~\ref{fig:openbox_and_closedbox} and \ref{fig:openbox_dave} are remarkably similar for $M_\text{star}>M_\text{star,max}$, quickly dropping as $M_\text{star}$ increases. This is not surprising, as in both cases the inflow of gas is almost zero for massive galaxies ($\xi_\text{step}=0$ for the former, $\xi_\text{Davé}\approx0$ for the latter). 
