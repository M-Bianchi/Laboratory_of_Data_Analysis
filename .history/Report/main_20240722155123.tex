...
Another difference is that, while for the analytical case $M_\text{gas}(t_0)$ is a free parameter that we have to choose, in the numerical approach it is an automatic result of time evolution from the time of formation up to when $\xi_\text{Davé}\approx0$ (i.e., when $M_\text{star}(t)\approx M_{\text{star,max}}$).

On the other hand, the results are quite different for actively star-forming galaxies, i.e., for $M_\text{star}<M_\text{star,max}$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
