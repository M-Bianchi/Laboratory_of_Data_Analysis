...
The main difference between the two cases is the presence of gas outflow: we set $\eta=0$ for the analytical case and $\eta=1.3$ for the numerical case. We could further generalize the time evolution by implementing $\eta=\eta(t)$ and $R=R(t)$ according to star-evolution physics, as they should decrease over time.
Another difference is that, while for the analytical case $M_\text{gas}(t_0)$ is a free parameter that we have to choose, in the numerical case it is simply a result of the evolution from the formation time up to when $\xi_\text{Davé}\approx0$ (i.e., when $M_\text{star}(t)\approx M_{\text{star,max}}$).
