...
\begin{equation}
	...
	\label{eq:mdotin}
\end{equation}
where $\xi \in [0,1]$ is a function related to the cooling efficiency discussed in Sec.~\ref{sec:introduction} and we will talk at length about it. We assume $f_\text{gas} = 0.15$, which is reasonable. $\dot{M}_\text{halo}^\text{in}(t)$ is still unknown to us, but we can assume an empirical relation found by \citet{McBride_2009}:
\begin{equation}
    \dot{M}_\text{halo}^\text{in}(t) = 42 M_{\odot} \text{yr}^{-1} \left(\dfrac{M_\text{halo}}{10^{12}M_{\odot}}\right)^{1.127} (1+1.17z) \sqrt{(1+z)^3 \Omega_m + \Omega_\Lambda}.
	\label{eq:mcbride}
\end{equation}
