...
\begin{figure}
    ...
    \caption{Fig. 8.6 from \citet{galaxy_formation_and_evolution_2010}. Cooling diagram showing the locus of $\tau_{\text{cooling}} = \tau_{\text{free-fall}}$ in the $n-T$ plane. The upper and lower curves correspond to gas with zero and solar metallicity, respectively. The tilted dashed lines are lines of constant gas mass (in Mo), while the horizontal dotted lines show the gas densities expected for virialized halos (8 = 200) at different redshifts. All calculations assume fgas = 0.15 2m,0 = 0.3, and h = 0.7. Cooling is effective for clouds with n and T above the locus.}
