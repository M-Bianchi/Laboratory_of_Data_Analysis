...
We generate a population of galaxies that formed at different times $t_\text{form,i}$, all with initial halo mass $M_\text{halo}(t_\text{form,i})=5 \times 10^8 M_\odot$. Then, we evolve them over infinitesimal time steps, increasing $M_\text{halo}$ according to eq.~\ref{eq:mcbride} and $M_\text{gas}$ eq.~\ref{eq:mgas_inflow}
