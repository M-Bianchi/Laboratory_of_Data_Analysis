...
\begin{equation}
	...
	\label{eq:mdotin}
\end{equation}
where $\xi \in [0,1]$ is a function related to the cooling efficiency discussed in Sec.~\ref{sec:introduction}, while $f_\text{gas}$ can be assumed $=0.15$. $\dot{M}_\text{halo}^\text{in}(t)$ is still unknown to us, but we can assume a relation found through the \textbf{Millenium Simulation} \citep{McBride_2009}:
\begin{equation}
