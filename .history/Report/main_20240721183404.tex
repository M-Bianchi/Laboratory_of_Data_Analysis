...
\begin{equation}
    M_\text{gas}(t) = M_\text{gas}(t_0)\left[1 - (1-R)\dfrac{M_\text{star}(t) - M_\text{star}(t_0)}{M_\text{gas}(t_0)}\right].
	\label{eq:mgas2_closedbox}
\end{equation}
The last step consists in substituting $M_{\text{gas}}$ in eq.~\ref{eq:SFR_vs_gasmas} and dividing by $M_{\text{star}}$:
\begin{equation}
    sSFR(t) = -(1-R)\epsilon' + \epsilon' \dfrac{M_\text{gas}(t_0) + (1-R)M_\text{star}(t_0)}{M_\text{star}(t)}.
