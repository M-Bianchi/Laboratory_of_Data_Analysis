...
The results in Figs.~\ref{fig:openbox_and_closedbox} and \ref{fig:openbox_dave} are remarkably similar for $M_\text{star}>M_\text{star,max}$. This is not surprising, as both approximate the passive galaxies with a closed-box model. The main difference between the two is that, while for the analytical case $M_\text{gas}(t_0)$ is a free parameter, in the numerical approach it is the result of the time evolution from the time of formation to when $\xi \approx 0$, i.e., when $M_\text{star}(t)\approx M_{\text{star,max}}$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
