...
\end{equation}
The last step consists in substituting eq.~\ref{eq:mgas2_closedbox} in eq.~\ref{eq:SFR_vs_gasmas} and, finally, dividing by $M_{\text{star}}$:
\begin{equation}
...
\end{equation}
We approximate $M_\text{star}(t_0)$ with $M_\text{star,max}$, since this is the threshold past which we can consider our closed-box model valid.
