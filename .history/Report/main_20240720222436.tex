...
\begin{equation}
	...
	\label{eq:mdotin}
\end{equation}
where $\xi \in [0,1]$ is a function related to the cooling efficiency discussed in Sec.~\ref{sec:introduction} and we will talk at length about it. $\dot{M}_\text{halo}^\text{in}(t)$ is still unknown to us, but we can assume an empirical relation found by \citet{McBride_2009}:
\begin{equation}
