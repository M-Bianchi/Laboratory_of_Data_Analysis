...
We could relax further approximations by considering more outflow processes. Even though Type IIA supernovae are the dominant driver for gas ejection, AGN emission and ram-pressure stripping in dense environments are both relevant processes that should be taken into account. For example, we could add a $\dot{M}_\text{gas}^\text{out, AGN} \propto M_\text{SMBH} \propto M_\text{halo}$ term for the observed galaxies that host an AGN. We can determine this by 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
