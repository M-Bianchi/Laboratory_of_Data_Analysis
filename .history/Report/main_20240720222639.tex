...
\begin{equation}
	...
	\label{eq:mdotin}
\end{equation}
where $\xi \in [0,1]$ is a function related to the cooling efficiency discussed in Sec.~\ref{sec:introduction}. We assume $f_\text{gas} = 0.15$, which is reasonable. $\dot{M}_\text{halo}^\text{in}(t)$ is still unknown to us, but we can assume an empirical relation found by \citet{McBride_2009}:
\begin{equation}
