...
The results in Figs.~\ref{fig:openbox_and_closedbox} and \ref{fig:openbox_dave} are remarkably similar for $M_\text{star}>M_\text{star,max}$. This is not surprising, as in both cases inflow of gas is almost zero for massive galaxies ($\xi_\text{step}=0$ for the former, $\xi_\text{Davé}\approx0$ for the latter). 
The main difference between the two is the presence of gas outflow: we set $\eta=0$ for the analytical case and $\eta=1.3$ for the numerical case. We could further generalize the time evolution by implementing a $\eta(t)$ that decreases as time goes by, as star-evolution physics suggests.
