...
On the other hand, the results are quite different for actively star-forming galaxies, i.e., for $M_\text{star}<M_\text{star,max}$, even though both of them fit at least a region of the main sequence. In the analytical case, we had to assume $\dot{M}_\text{gas}^\text{in}=\dot{M}_\text{gas}^\text{out}$ at the time of observation, in order to solve the differential equation. On the contrary, numerical evolution is more general, as outflow and inflow can change freely. For this reason, we think that the numerical result is more trustworthy.
