...
The results in Figs.~\ref{fig:openbox_and_closedbox} and \ref{fig:openbox_dave} are remarkably similar for $M_\text{star}>M_\text{star,max}$. This is not surprising, as in both cases inflow of gas is almost zero for massive galaxies ($\xi_\text{step}=0$ for the former, $\xi_\text{Davé}\approx0$ for the latter). The main difference between the two is that, while we set $\eta=0$ for the analytical case, in the numerical time evolution $\eta$ is also constant and positive ($\eta=1.3$). We could further generalize this by making $\eta(t)$ change over time, as physics suggests.
