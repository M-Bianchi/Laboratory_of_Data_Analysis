...
On the other hand, the results are quite different for actively star-forming galaxies, i.e., for $M_\text{star}<M_\text{star,max}$. In the analytical case, in order to solve the differential equation, we had to assume $\dot{M}_\text{gas}^\text{in}=\dot{M}_\text{gas}^\text{out}$ at the time of observation, while there is no need for this in the numerical evolution.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
