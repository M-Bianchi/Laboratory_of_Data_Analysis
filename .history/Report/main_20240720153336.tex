...
This work is based on a subset of the \textbf{Sloan Digital Sky Survey} (SDSS). In particular, we study $92483$ galaxies with redshift $< 0.08$ ($z_{\text{mean}} \simeq 0.054$), for which we have five-band photometric data. This data may look too sparse to obtain a valuable estimation of the SFR, but we have been able to achieve this by employing a very sophisticated library called \textbf{CIGALE} (Code Investigating GALaxy Emission). It generates a grid of model parameters and fits the photometric data with each model, selecting the one that maximizes the likelihood.

We choose to (1) model the star formation history (SFH) with a double exponential, (2) employ the initial mass function (IMF) by \citet{Chabrier_2003}, (3) model the dust attenuation with \citet{Calzetti_2000} and  \citet{Leitherer_2002} formulae, and (4) to take into account also nebular emission.
