...
{\fontsize{8.2pt}{8.2pt}\begin{equation}
    M^\text{eq}_\text{gas}\left(M_{\text{star}}\right) = \dfrac{\dot{M}_\text{gas}^\text{in}\left(M_{\text{star}}\right)}{(1+\eta-R)\epsilon'} \: \Rightarrow \: sSFR^\text{eq}(t) = \dfrac{\dot{M}_\text{gas}^\text{in}(t)}{1+\eta-R} \, \dfrac{1}{M_\text{star}(t)},
	\label{eq:openbox_equilibrium}
\end{equation}}
where $\dot{M}_\text{gas}^\text{in}(t)$ is given by \cref{eq:mgas_inflow,eq:xi_step,eq:mcbride,eq:moster}, while $\eta$ and $R$ are free parameters of the model.
