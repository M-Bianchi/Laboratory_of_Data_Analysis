...
\begin{equation}
	...
	\label{eq:moster}
\end{equation}
where $N$, $\beta$, $\gamma$, and $M_1$ are coefficients that can be found in the original paper. We can use this relation to transform the efficient cooling condition of eq.~\ref{eq:halo_mass_minmax} to a requirement on $M_\text{star}$:
\begin{equation}
